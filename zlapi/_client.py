...
class ZaloAPI(object):
   ...
   def sendMessage(self, message, thread_id, thread_type, mark_message=None, ttl=0):
      ...
      if thread_type == ThreadType.USER:
         try:
            ...
            payload["params"]["toIds"] = payload["params"].pop("grids")
            payload["params"]["toIds"][0]["toUid"] = str(thread_id)
            payload["params"]["toIds"]["imei"] = self._imei
         except:
            bug = "cocaiconcacemnhe"
      elif thread_type == ThreadType.GROUP:
         url = "https://tt-files-wpa.chat.zalo.me/api/group/mforward"
         payload["params"]["visibility"] = 0
         payload["params"]["grids"][0]["grid"] = str(thread_id)
      else:
         raise ZaloUserError("Thread type is invalid")
   ...
   def sendCustomSticker(self, staticImgUrl, animationImgUrl, thread_id, thread_type, ttl=0, width=0, height=0):
